...
class AddRelationCustomizeAdapter(
   private val menuAddOne: Int?,
    ...
    ) : RecyclerView.Adapter<AddRelationCustomizeAdapter.RelationHolder>() {
    ...
    override fun onBindViewHolder(holder: RelationHolder, position: Int) {
        ...
        holder.itemName.text = item.itemName + ", CZK ${PrefUtils.instance.formatDouble(item.price ?: 0.0)}"
            ...
            if (chooseUpto != 0){
                if (required){
                    if (!item.isSelected!!){
                        ...
                        updateCart(item.price ?: 0.0,item.id ?: 0,true,menuAddOne)
                        ...
                        updateCart(item.price ?: 0.0,item.id ?:0,false,menuAddOne)
...
// }
        holder.clickRadio.setOnClickListener {
            addOnsRelations.forEach {
                if (it.isSelected!!){
                    requiredCounterValue++
                    updateCart(it.price ?: 0.0,null,false,menuAddOne)
                    it.isSelected = false
                }
            }
            requiredCounterValue--
